...
from .utils import abs_square, tensor_H
...
def fastiva(
    ...
    **kwargs,
    ...
    W = np.zeros((n_freq, n_src, n_chan), dtype=X.dtype)
    W[:] = np.eye(n_chan)[None, :n_src, :]
    np.matmul(W, X, out=Y)
    ...
    for epoch in range(n_iter):
        ...
        # shape: (n_freq, n_src, n_frames)
        Y_sq = abs_square(Y)

        # shape: (1, n_src, n_frames)
        ...
        WWH = W @ tensor_H(W)
        WWH = 0.5 * (WWH + tensor_H(WWH))
        e_val, e_vec = np.linalg.eigh(WWH)
        L = (
            e_vec * np.reciprocal(np.maximum(np.sqrt(e_val[:, None, :]), 1e-15))
        ) @ tensor_H(e_vec)
        np.matmul(L, W, out=W)
        ...
        if callback is not None and (epoch + 1) in callback_checkpoints:
            Y_tmp = Y.transpose([2, 0, 1])
            callback(W @ Q_H_inv, Y_tmp.copy(), model)
